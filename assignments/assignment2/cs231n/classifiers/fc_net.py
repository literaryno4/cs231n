...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dout2, grads['W2'], grads['b2'] = affine_backward(dscore, cache2)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        if self.use_dropout:
            cache_dp = [None] * (self.num_layers + 1)
        h[0] = X
        for i in range(self.num_layers - 1):
            if self.normalization == 'batchnorm':
                ...
                h[i+1], cache[i+1] = affine_bn_relu_forward(h[i], W, b, gamma, beta, bn_param)
            elif self.normalization == 'layernorm':
                W, b = self.params['W'+str(i+1)], self.params['b'+str(i+1)]
                gamma,  beta = self.params['gamma'+str(i+1)], self.params['beta'+str(i+1)]
                bn_param = self.bn_params[i]
                h[i+1], cache[i+1] = affine_ln_relu_forward(h[i], W, b, gamma, beta, bn_param)
            else:
                W, b = self.params['W'+str(i+1)], self.params['b'+str(i+1)]
                h[i+1], cache[i+1] = affine_relu_forward(h[i], W, b)
            if self.use_dropout:
                h[i+1], cache_dp[i+1] = dropout_forward(h[i+1], self.dropout_param)
        else:
            i += 1
            W, b = self.params['W'+str(i+1)], self.params['b'+str(i+1)]
        ...
        while i > 0:
            if self.use_dropout:
                dh[i] = dropout_backward(dh[i], cache_dp[i])
            if self.normalization == 'batchnorm':
                dh[i-1], grads['W'+str(i)], grads['b'+str(i)], grads['gamma'+str(i)], grads['beta'+str(i)] = affine_bn_relu_backward(dh[i], cache[i])
            elif self.normalization == 'layernorm':
                dh[i-1], grads['W'+str(i)], grads['b'+str(i)], grads['gamma'+str(i)], grads['beta'+str(i)] = affine_ln_relu_backward(dh[i], cache[i])
            else:
